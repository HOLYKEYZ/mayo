                ...
                try:
                    ...
                    if improvement_data and 'edits' in improvement_data:
                            ...
                            if file_changes:
                                ...
                                if success:
                                    # DISABLED: PR creation triggers email to watchers
                                    # pr = issue_repo.create_pull(
                                    #     title=f"{title}",
                                    #     body=f"{improvement_data.get('body', '')}\n\n**Automated improvement PR.**",
                                    #     head=branch,
                                    #     base=issue_repo.default_branch,
                                    #     draft=True
                                    # )
                                    # print(f"DEBUG: PR created from approved issue: {pr.html_url}")
                                    print(f"DEBUG: Would create PR from approved issue (disabled - email spam)")
...
Output ONLY JSON: {{"reasonableness_score": 70, "action": "fix|close|skip", "reason": "explanation", "fix_plan": "if action=fix, describe what to change"}}"""
        ...
        if should_run_timed_phase(current_memory, 'LAST_PROACTIVE_ISSUE', 24):
            ...
            try:
                # All non-fork repos except blacklisted
...
# === REPO SELECTION: ALL REPOS EXCEPT BLACKLISTED ===
        ...
        if 'DIRECTIVE: OPEN_ISSUE' in scanner_plan:
            ...
            try:
                ...
                try:
                    ...
                    note = f"\n- **Repo: {target_repo.name}**: Scanner wanted to open issue — {issue_title} (disabled to prevent email spam)"
                    bot_repo.update_file("data/global_memory.md", co_author_msg(f"feat(memory): scanner identified issue on {target_repo.name}"), old_mem + note, mem_file.sha)
                except Exception as e:
        ...
        if success:
            ...
            #     title=f"{final_title}",
            #     body=f"{final_body}\n\n*Automated improvement by Triple-AI pipeline.*",
            #     head=final_branch,
            #     base=target_repo.default_branch,
            #     draft=True
            # )
            # print(f"DEBUG: PR created: {pr.html_url}")
            print(f"DEBUG: Would create PR for {target_repo.name} (disabled - email spam)")
            ...
            try:
                ...
                lesson = f"\n- **Repo: {target_repo.name}**: {final_title}. (Ref: disabled - email spam) - *Status: SKIPPED*"
